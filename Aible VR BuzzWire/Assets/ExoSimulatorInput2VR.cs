...
namespace PW
{
    public class ExoSimulatorInput2VR : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            //HomePosition = new Vector3(0.0f, 0.0f, 0.0f);
            ...
            if (VRWrist != null)
            {
                ...
                if(string.Equals(msgCommand, "ExoData"))
                {
                    ...
                    //HomePosition = VRWrist.gameObject.transform.position;

                    //Debug.Log(VRWrist.gameObject.transform.position);
                    ...
                    if (VRCube != null)
                    {
                        ...
                        nextRotation = VRCube.gameObject.transform.localRotation;
                    }
            ...
            MaxPathChildren = trajPath.transform.childCount;
            numPathChildren = 0;
            nxtPathPos = trajPath.transform.GetChild(numPathChildren).position;
            //nxtPathQu = trajPath.transform.GetChild(numPathChildren).localRotation;
            nxtPathQu = Quaternion.identity;
            //Debug.Log(nxtPathPos);
            FirstTime = true;
        }
        ...
        void CheckForVRArmUpdate()
        {
            ...
            ////string msgarm = this.client.ReceiveFrameString();
            ////ProcessArmUpdate(msgarm);
        ...
        void Update()
        {
            ...
            if (deltaTime >= interval)
            {
                ...
                if (FirstTime)
                {
                    ...
                    nextPosition = nxtPathPos;
                    nextRotation = nxtPathQu;

                    Debug.Log(nextPosition);

                    var dis = Vector3.Distance(m_JointTransfrom[3].position, nextPosition); // Calculating Distance
                    if (dis <= 0.05f) // checking if distance is less than required distance.
                    {
                        Debug.Log("Next Path Pos");
                        numPathChildren++;
                        Debug.Log(numPathChildren);
                        GetNextPathPos();
        ...
        void GetNextPathPos()
        {
            ...
            if (numPathChildren < MaxPathChildren)
            {
                nxtPathPos = trajPath.transform.GetChild(numPathChildren).position;
                nxtPathQu = trajPath.transform.GetChild(numPathChildren).localRotation;
                //nxtPathQu = Quaternion.identity;
